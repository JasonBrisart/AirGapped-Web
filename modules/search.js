(function () {
    ...
    function buildSearchText(page, website, snapshot) {
        return [
            ...
            website && Array.isArray(website.tags) ? website.tags.join(" ") : "",
    ...
    function renderSearch(inputId, resultsId) {
        ...
        function runSearch() {
            ...
            const matches = query
                ? data.pages.filter(function (page) { return pageMatchesQuery(page, query, data); })
                : data.pages.slice();
            ...
            for (const page of matches) {
                html += renderSearchResult(page, core.byId(data.websites, page.website_id));
            }
    ...
    AGW.pageMatchesQuery = function (page, query) {
        return pageMatchesQuery(page, query, core.getData());
    };
